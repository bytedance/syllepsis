...
interface ITextShortcutPlugin extends Plugin {
  getStatus: () => boolean;
}
...
const createTextShortcutPlugin = (_textShortcuts: TextShortcut[], _enable = true) => {
  ...
  const textShortPlugin = new Plugin({
    key: SHORTCUT_KEY,
    state: {
      init(): ShortCutState {
        return null;
  ...
  }) as ITextShortcutPlugin;

  textShortPlugin.getStatus = () => enable;
  return textShortPlugin;
};
